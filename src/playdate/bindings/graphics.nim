...
# Video
type PlaydateVideoRaw {.importc: "struct playdate_video", header: "pd_api.h".} = object

type PlaydateVideo* = ptr PlaydateVideoRaw

# Graphics
sdktype PlaydateGraphics:
    type PlaydateGraphics {.importc: "const struct playdate_graphics", header: "pd_api.h".} = object
        video* {.importc: "video".}: PlaydateVideo
        ...
        setFontRaw {.importc: "setFont".}: proc (font: LCDFontPtr) {.cdecl, raises: [].}

        setTextTracking* {.importsdk.}: proc (tracking: cint)
        pushContextRaw {.importc: "pushContext".}: proc (target: LCDBitmapPtr) {.cdecl, raises: [].}
        popContext* {.importc: "popContext".}: proc () {.cdecl, raises: [].}
        drawBitmapRaw {.importc: "drawBitmap".}: proc (bitmap: LCDBitmapPtr; x: int; y: int;
            flip: LCDBitmapFlip) {.cdecl, raises: [].}
        
        tileBitmapRaw {.importc: "tileBitmap".}: proc (bitmap: LCDBitmapPtr; x: cint; y: cint;
        ...
        drawScaledBitmapRaw {.importc: "drawScaledBitmap".}: proc (bitmap: LCDBitmapPtr;
        ...
        newBitmapRaw {.importc: "newBitmap".}: proc (width: cint; height: cint;
            bgcolor: LCDColor): LCDBitmapPtr {.cdecl, raises: [].}
        freeBitmapRaw {.importc: "freeBitmap".}: proc (bitmap: LCDBitmapPtr) {.cdecl, raises: [].}
        loadBitmapRaw {.importc: "loadBitmap".}: proc (path: cstring; outerr: ptr cstring): LCDBitmapPtr {.
            cdecl, raises: [].}

        copyBitmapRaw {.importc: "copyBitmap".}: proc (bitmap: LCDBitmapPtr): LCDBitmapPtr {.
            cdecl, raises: [].}
        loadIntoBitmapRaw {.importc: "loadIntoBitmap".}: proc (path: cstring;
            bitmap: LCDBitmapPtr; outerr: ptr cstring) {.cdecl, raises: [].}
        getBitmapDataRaw {.importc: "getBitmapData".}: proc (bitmap: LCDBitmapPtr;
            width: ptr cint; height: ptr cint; rowbytes: ptr cint; mask: ptr ptr uint8;
            data: ptr ptr uint8) {.cdecl, raises: [].}
        clearBitmapRaw {.importc: "clearBitmap".}: proc (bitmap: LCDBitmapPtr;
            bgcolor: LCDColor) {.cdecl, raises: [].}
        rotatedBitmapRaw {.importc: "rotatedBitmap".}: proc (bitmap: LCDBitmapPtr;
            rotation: cfloat; xscale: cfloat; yscale: cfloat; allocedSize: ptr cint): LCDBitmapPtr {.
            cdecl, raises: [].}
        
        newBitmapTableRaw {.importc: "newBitmapTable".}: proc (count: cint; width: cint;
            height: cint): LCDBitmapTablePtr {.cdecl, raises: [].}
        freeBitmapTableRaw {.importc: "freeBitmapTable".}: proc (table: LCDBitmapTablePtr) {.
            cdecl, raises: [].}
        loadBitmapTableRaw {.importc: "loadBitmapTable".}: proc (path: cstring;
            outerr: ptr cstring): LCDBitmapTablePtr {.cdecl, raises: [].}
        loadIntoBitmapTableRaw {.importc: "loadIntoBitmapTable".}: proc (path: cstring;
            table: LCDBitmapTablePtr; outerr: ptr cstring) {.cdecl, raises: [].}
        getTableBitmapRaw {.importc: "getTableBitmap".}: proc (table: LCDBitmapTablePtr;
            idx: cint): LCDBitmapPtr {.cdecl, raises: [].}        

        loadFontRaw {.importc: "loadFont".}: proc (path: cstring, outErr: ptr cstring): LCDFontPtr {.cdecl, raises: [].}

        getFontPageRaw {.importc: "getFontPage".}: proc (font: LCDFontPtr; c: uint32): LCDFontPagePtr {.
            cdecl, raises: [].}
        getPageGlyphRaw {.importc: "getPageGlyph".}: proc (page: LCDFontPagePtr; c: uint32;
            bitmap: ptr LCDBitmapPtr; advance: ptr cint): LCDFontGlyphPtr {.cdecl, raises: [].}
        getGlyphKerningRaw {.importc: "getGlyphKerning".}: proc (glyph: LCDFontGlyphPtr;
            glyphcode: uint32; nextcode: uint32): cint {.cdecl, raises: [].}

        getTextWidthRaw {.importc.}: proc (font: LCDFontPtr, text: cstring;
            len: csize_t; encoding: PDStringEncoding; tracking: cint): cint {.cdecl, raises: [].}

        getFrameRaw {.importc: "getFrame".}: proc (): ptr uint8 {.cdecl, raises: [].} ##  row stride = LCD_ROWSIZE
        getDisplayFrameRaw {.importc: "getDisplayFrame".}: proc (): ptr uint8 {.cdecl, raises: [].} ##  row stride = LCD_ROWSIZE
        getDebugBitmapRaw {.importc: "getDebugBitmap".}: proc (): LCDBitmapPtr {.cdecl, raises: [].} ##  valid in simulator only, function is NULL on device
        copyFrameBufferBitmapRaw {.importc: "copyFrameBufferBitmap".}: proc (): LCDBitmapPtr {.
            cdecl, raises: [].}
        markUpdatedRows {.importsdk.}: proc (start: cint; to: cint)
        display* {.importsdk.}: proc () ##  misc util.
        
        setColorToPatternRaw {.importc: "setColorToPattern".}: proc (color: ptr LCDColor;
        ...
        fillPolygonRaw {.importc: "fillPolygon".}: proc (nPoints: cint; coords: ptr cint;
            color: LCDColor; fillrule: LCDPolygonFillRule) {.cdecl, raises: [].}
        getFontHeightRaw {.importc: "getFontHeight".}: proc (font: LCDFontPtr): uint8 {.
            cdecl, raises: [].}               ##  1.7
        getDisplayBufferBitmapRaw {.importc: "getDisplayBufferBitmap".}: proc (): LCDBitmapPtr {.
            cdecl, raises: [].} # system owned, don't free.
        drawRotatedBitmapRaw {.importc: "drawRotatedBitmap".}: proc (
        ...
        setBitmapMaskRaw {.importc: "setBitmapMask".}: proc (bitmap: LCDBitmapPtr;
            mask: LCDBitmapPtr): cint {.cdecl, raises: [].}
        getBitmapMaskRaw {.importc: "getBitmapMask".}: proc (bitmap: LCDBitmapPtr): LCDBitmapPtr {.
            cdecl, raises: [].}               ##  1.10
        setStencilImageRaw {.importc: "setStencilImage".}: proc (stencil: LCDBitmapPtr;
            tile: cint) {.cdecl, raises: [].}   ##  1.12
        makeFontFromDataRaw {.importc: "makeFontFromData".}: proc (data: LCDFontDataPtr;
